...
export const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (loginType === 'password') {
      ...
      setLoading(false);
      if (res?.success) {
        if (res.user.role === 'HOD') navigate('/inventory/items');
        else navigate('/');
      }
    } else {
      // OTP mode
  ...
  const handleQuickLogin = async (type) => {
    ...
    if (type === 'admin') {
      ...
      const res = await login('admin@library.com', 'Admin@123');
      if (res?.success) navigate('/');
    } else if (type === 'librarian') {
      setEmail('librarian@library.com');
      setPassword('Librarian@123');
      const res = await login('librarian@library.com', 'Librarian@123');
      if (res?.success) navigate('/');
    } else if (type === 'hod') {
      setEmail('hod@library.com');
      setPassword('Hod@123');
      const res = await login('hod@library.com', 'Hod@123');
      if (res?.success) navigate('/inventory/items');
    } else {
      setEmail('LIB-00001');
      setPassword('LIB-00001');
      const res = await login('LIB-00001', 'LIB-00001');
      if (res?.success) navigate('/');
    }
    setLoading(false);
  };
        ...
        <div className="pt-2 border-t border-slate-100 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 py-2.5 px-3 bg-[#F3F5F9] hover:bg-indigo-50 hover:text-indigo-600 text-slate-700 font-bold text-xs rounded-2xl transition shadow-sm"
            >
